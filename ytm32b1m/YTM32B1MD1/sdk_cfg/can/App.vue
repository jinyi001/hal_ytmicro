            ...
            >
              ...
              >
                <el-collapse-item
                  ...
                  name="1"
                >
                  <!-- Button to trigger the CAN Phase Configuration dialog -->
                  <el-button
                    type="primary"
                    @click="openCanPhaseDialog('nominal')"
                  >
                    Configure CAN Phase
                  </el-button>

                  <!-- CAN Phase Configuration Dialog -->
                  <el-dialog
                    v-model="canPhaseDialogVisible"
                    :title="calculationType === 'nominal' ? 'CAN Phase Configuration' : 'CAN FD Phase Configuration'"
                    width="70%"
                    align-center
                  ...
                  >
                    <div>
                      <el-form :inline="true">
                        <el-form-item label="Baud Rate (Kbps)">
                          <el-input
                            v-model="baudRate"
                            placeholder="Enter Baud Rate (e.g. 500)"
                          />
                        </el-form-item>
                      ...
                      <!-- Display possible configurations -->
                      <div v-if="phaseConfigs.length > 0">
                        <el-table
                          :data="phaseConfigs"
                          style="width: 100%"
                          height="400"
                          highlight-current-row
                          @row-dblclick="handlePhaseConfigDblClick"
                    ...
                    <template #footer>
                      <span class="dialog-footer">
                        <el-button @click="canPhaseDialogVisible = false">Cancel</el-button>
                        <el-button
                          type="primary"
                          @click="applyPhaseConfig"
                          :disabled="!selectedPhaseConfig"
                        >Apply</el-button>
                      </span>
                    </template>
                  </el-dialog>
                ...
                >
                  <el-button
                    type="primary"
                    @click="openCanPhaseDialog('data')"
                    :disabled="!data[activeMasterInst].fd_enable"
                    style="margin-bottom: 20px;"
                  >
                    Configure CAN FD Phase
                  </el-button>
                  <el-form-item
                    label="Propagation segment"
...
const canPhaseDialogVisible=ref(false)
const baudRate = ref(500)
const phaseConfigs = ref<any[]>([])
const selectedPhaseConfig = ref<any>(null)

const calculationType = ref<'nominal' | 'data'>('nominal')

function openCanPhaseDialog(type: 'nominal' | 'data' = 'nominal') {
  calculationType.value = type
  canPhaseDialogVisible.value = true
  // Reset or initialize if needed
  phaseConfigs.value = []
  selectedPhaseConfig.value = null
  // Default baud rate suggestion?
  baudRate.value = type === 'nominal' ? 500 : 2000
}

function calculateCanPhase() {
  phaseConfigs.value = []
  const targetBaud = Number(baudRate.value) // Kbps
  if (!targetBaud || targetBaud <= 0) return

  // freq is in Hz, e.g. 80000000
  const f_sys = freq.value 
  if (!f_sys) return

  let maxPrescaler = 256
  let minProp = 1
  let maxProp = 8
  let minSeg1 = 1
  let maxSeg1 = 8
  ...
  // Override limits for Data Phase (CBT)
  if (calculationType.value === 'data') {
      maxPrescaler = 1024
      minProp = 0
      maxProp = 31
      minSeg1 = 1
      maxSeg1 = 8 // Based on validation rules
      minSeg2 = 1
      maxSeg2 = 8 // Based on validation rules
      maxSJW = 8  // Based on validation rules
  }

  // Common TQ limits (derived from min/max of segments)
  // Min TQ = 1 (Sync) + minProp + minSeg1 + minSeg2
  const minTQ = 1 + minProp + minSeg1 + minSeg2 
  // Max TQ = 1 (Sync) + maxProp + maxSeg1 + maxSeg2
  const maxTQ = 1 + maxProp + maxSeg1 + maxSeg2

  for (let prescaler = 1; prescaler <= maxPrescaler; prescaler++) {
    // TotalTQ = (f_sys / 1000) / (Prescaler * Baud)
    // Use Math.round to check primarily for integer result
    const numerator = f_sys / 1000
    const denominator = prescaler * targetBaud
    
    // Check if integer (allow small float error)
    if (Math.abs((numerator / denominator) - Math.round(numerator / denominator)) > 0.0001) continue
    
    const totalTQ = Math.round(numerator / denominator)
    
    if (totalTQ < minTQ || totalTQ > maxTQ) continue

    // totalTQ = 1 + PropSeg + PhaseSeg1 + PhaseSeg2
    for (let propSeg = minProp; propSeg <= maxProp; propSeg++) {
      for (let phaseSeg1 = minSeg1; phaseSeg1 <= maxSeg1; phaseSeg1++) {
        const phaseSeg2 = totalTQ - 1 - propSeg - phaseSeg1
        
        if (phaseSeg2 >= minSeg2 && phaseSeg2 <= maxSeg2) {
             const samplePointVal = (1 + propSeg + phaseSeg1) / totalTQ * 100
             
             // Heuristic for SJW (max usually <= PhaseSeg1, but limited by hardware max)
             const sjw = Math.min(maxSJW, phaseSeg1, phaseSeg2)

             phaseConfigs.value.push({
               preDivider: prescaler,
               propSeg: propSeg,
               phaseSeg1: phaseSeg1,
               phaseSeg2: phaseSeg2,
               rJumpwidth: sjw,
               samplePoint: Number(samplePointVal.toFixed(2)),
               id: `${prescaler}-${propSeg}-${phaseSeg1}-${phaseSeg2}`
             })
        }
      }
    }
  }
  // Sort by sample point ascending
  phaseConfigs.value.sort((a, b) => a.samplePoint - b.samplePoint)
}

function applyPhaseConfig() {
  if (selectedPhaseConfig.value && data.value[activeMasterInst.value]) {
    const target = calculationType.value === 'nominal' 
      ? data.value[activeMasterInst.value].bitrate 
      : data.value[activeMasterInst.value].bitrate_cbt

    target.preDivider = selectedPhaseConfig.value.preDivider
    target.propSeg = selectedPhaseConfig.value.propSeg
    target.phaseSeg1 = selectedPhaseConfig.value.phaseSeg1
    target.phaseSeg2 = selectedPhaseConfig.value.phaseSeg2
    target.rJumpwidth = selectedPhaseConfig.value.rJumpwidth
    canPhaseDialogVisible.value = false
  }
}

function handlePhaseConfigDblClick(row: any) {
    selectedPhaseConfig.value = row
    applyPhaseConfig()
}

function handleCurrentChange(val: any) {
    selectedPhaseConfig.value = val
}
