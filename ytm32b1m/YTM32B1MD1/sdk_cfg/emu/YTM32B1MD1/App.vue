...
function addrValid(rule: any, value: any, callback: any, source: any) {
  ...
  if (val.channel == 0) {
    if (addr >= 0x1FFF8000 && addr <= 0x1FFFFFFF) {
      callback()
    } else {
      callback(new Error('Addr should from 0x1FFF_8000 - 0x1FFF_FFFF'))
    }
  }
  if (val.channel == 1) {
    if (addr >= 0x2000_0000 && addr <= 0x2000_7FFF) {
      callback()
    } else {
...
const MasterRules = reactive<FormRules>({
  ...
  channel: [
    ...
    { type: 'number', min: 0, max: 1, message: 'Channel from 0-1', trigger: 'change' }
  ],
  injectErrAddr: [
