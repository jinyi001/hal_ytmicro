...
function resetTableRowWidth() {
  const tableHeaderDom = document.querySelector('.el-table-v2__header');
  if (tableHeaderDom) {
    const cs = getComputedStyle(tableHeaderDom);
    headerW.value = cs.width;
    return true;
  }
  return false;
}
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }) => {
      ...
      return h('select', {
        ...
        'onChange': (event) => rowData.filter = event.target.value == 'true',
        style: { width: '100%' }
      }, [h('option', {
        label: "Disabled",
        value: false
    ...
    if (val != "") {
      ...
      if (err == false) {
        ...
        if (tableIndex == -1) {
          ...
          if (!["YTM32Z1ME0", "YTM32Z1MD0"].includes(chipName.value)) {
            changeAttr = {
              digitalWidth: 0,
              ...changeAttr
            }
  ...
  const checkOneCfg = (name: string, pins: PinCfg[])=>{
    ...
    switch (checkRes) {
      case PinCfgErrorType.OK:
        validPinCfgArray.push({
          name: pinConfigs[pinCfgIdx].name,
          pins: pinConfigs[pinCfgIdx].pins.filter(v=>v!=null)
        });
        break;
...
function addHoverEvent() {
  ...
  tableContainer.onmouseover = e=>{
    ...
    if (targetDom !== tableContainer) {
      ...
      if (isNaN(targetIndex) && targetIndex >= 0 && targetIndex <= pinsCfgArray.value.length) {
...
onMounted(() => {
  ...
  retryFunction(resetTableRowWidth);
});
...
<style>
.table .el-vl__wrapper.el-table-v2__body div:has(>.el-table-v2__row) {
  width: v-bind(headerW + 'px')!important;
}

.table .el-table-v2.is-dynamic .el-table-v2__row.table-row {
  width: v-bind(headerW + 'px')!important;
}
