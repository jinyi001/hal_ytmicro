...
void lpTMR0_IRQHandler(void)
{
    ...
    if (lptmrIsrCnt > 1)
    {
        ...
        lpTMR_DRV_SetCompareValueByCount(LPTMR_INST, 23437);
...
int main(void)
{
    ...
    /* USER CODE BEGIN 2 */
    PRINTF("Build %s %s\n", __DATE__, __TIME__);
    PRINTF("Lptmr Dynamic Demo!\r\n");
    INT_SYS_EnableIRQ(lpTMR0_IRQn);
    /* Start lpTMR counter */
    lpTMR_DRV_StartCounter(LPTMR_INST);
    /* Wait lpTMR enter interrput */
    while (lptmrIsrCnt < TEST_CNT)
        ;
    /* Stop lpTMR counter */
    lpTMR_DRV_StopCounter(LPTMR_INST);
    PRINTF("lpTMR Dynamic Update Demo Pass!\n");
    lpTMR_DRV_Deinit(LPTMR_INST);
    PRINTF("lpTMR Dynamic Update Demo End!\n");
    /* USER CODE END 2 */
