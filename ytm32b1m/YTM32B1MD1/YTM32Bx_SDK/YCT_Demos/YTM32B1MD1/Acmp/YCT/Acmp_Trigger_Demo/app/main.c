...
void ACMP0_IRQHandler(void)
{
    ...
    else
    {
        ...
        /* Channel 0 4V > channel 1 3V */
        acmpError = true;
    }
    /* Disable ACMP for PRINT result */
    acmpInt = true;
    ACMP_DRV_Disable(0);
}
...
int main(void)
{
    ...
    ACMP_DRV_Enable(0);
    /* Start lpTMR0 */
    PRINTF("lpTMR will trig ACMP in 1s.\n");
    lpTMR_DRV_StartCounter(0);
    /* Delay 100ms to check ACMP not enter INT */
    OSIF_TimeDelay(100);
    if (acmpInt || acmpError)
    {
        acmpError = true;
        PRINTF("ACMP Trigger Demo Failed.\n");
    }
    /* Delay 1s to check ACMP enter INT */
    OSIF_TimeDelay(1000);
    if (acmpInt && !acmpError)
    {
        PRINTF("ACMP Trigger Demo Pass.\n");
    }
    else
    {
        PRINTF("ACMP Trigger Demo Failed.\n");
