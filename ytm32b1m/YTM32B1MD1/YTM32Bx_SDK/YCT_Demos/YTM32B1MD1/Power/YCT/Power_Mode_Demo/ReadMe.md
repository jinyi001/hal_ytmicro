...
This example demonstrates the chip's capability to enter various low-power modes:

- When the MCU enters a non-Power Down mode (e.g., Standby mode), it can be woken up by a button press (PTC1) via a GPIO interrupt. The program resumes execution from the point of interruption.
- When the MCU enters Power Down mode, the user must configure the WKU module. Similarly, a button press (PTC1) triggers the WKU to wake up the MCU, resulting in a system reset.
