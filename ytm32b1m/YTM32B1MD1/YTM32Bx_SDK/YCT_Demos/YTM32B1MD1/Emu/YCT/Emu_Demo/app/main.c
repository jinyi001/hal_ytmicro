...
void EMU0_SB_IRQHandler(void)
{
    ...
    if (EMU_DRV_GetChannelSBInterruptFlag(0, 1))
    {
        EMU_DRV_ClearInterruptFlagSignalBit(0, 1);
    }
}
...
int main(void)
{
    ...
    /* Read initial value */
    initial = *(volatile uint32_t *)(EMU_Config0.injectErrAddr);

    INT_SYS_EnableIRQ(EMU0_SB_IRQn);
    /* Initialize Emu and Enable error injection. */
    ...
    Otherwise , the inject error will always be there, and read a time will trigger ecc error. */
    *(volatile uint32_t *)(EMU_Config0.injectErrAddr) = 0x01;

    if (value == initial)
    {
        PRINTF("Injection fault has been corrected!\n");
    }
    else
    {
        PRINTF("Injection fault has not been corrected!\n");
    }
    ...
    {
        PRINTF("EMU Demo Fail!\n");
    }
    else
    {
        PRINTF("EMU Demo Pass!\n");
    }
    PRINTF("EMU Demo End.\n");
    /* USER CODE END 2 */
