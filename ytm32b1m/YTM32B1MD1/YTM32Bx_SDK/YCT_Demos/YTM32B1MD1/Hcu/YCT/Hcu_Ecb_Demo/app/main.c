...
int main(void)
{
    ...
    /* Load AES key */
    status |= HCU_DRV_LoadUserKey(key, KEY_SIZE_128_BITS);
    PRINTF("ECB Encrypt using 128bit key test. \r\n");
    /* Start AES-ECB encrypt */
    status |= HCU_DRV_EncryptECB(plainText, 64, result);
    PRINTF("Plain Text:\n");
    for (uint32_t i = 0; i < 4; i++)
    {
        for (uint32_t j = 0; j < 4; j++)
        {
            PRINTF("%08X, ", plainText[4 * i + j]);
        }
        PRINTF("\n");
    }
    PRINTF("Cipher Text:\n");
    ...
    {
        PRINTF("AES-ECB Encrypt Demo Fail! \r\n");
    }
    else
    {
        PRINTF("AES-ECB Encrypt Demo Pass! \r\n");
    }
    HCU_DRV_DeInit(&hcu_config0_State);
    PRINTF("HCU ECB Demo End.\n");
    /* USER CODE END 2 */
