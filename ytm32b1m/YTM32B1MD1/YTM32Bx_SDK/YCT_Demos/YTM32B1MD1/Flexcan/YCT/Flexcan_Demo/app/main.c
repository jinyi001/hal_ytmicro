...
#define TX_EXTMSG_ID   (0x70012UL)
#define TX_EXT_MAILBOX (0x03UL)
...
/* CAN2 configuration */
/* Define receive buffer */
flexcan_msgbuff_t rxStdMsg2;
flexcan_msgbuff_t rxExtMsg2;
/* Prepare message to be sent */
...
int main(void)
{
    ...
    /* USER CODE BEGIN 2 */
    PRINTF("Build %s %s\n", __DATE__, __TIME__);
    PRINTF("FlexCAN transmit between CAN1 and CAN2 on EVB.\n");
    ...
    status |= FLEXCAN_DRV_ConfigRxMb(CAN1_INST, RX_EXT_MAILBOX, &rxMbExtInfo1, RX_EXTMSG_ID);

    status |= FLEXCAN_DRV_ConfigRxMb(CAN2_INST, RX_STD_MAILBOX, &rxMbStdInfo2, RX_STD_MSG_ID);
    status |= FLEXCAN_DRV_ConfigRxMb(CAN2_INST, RX_EXT_MAILBOX, &rxMbExtInfo2, RX_EXTMSG_ID);

    /* Start receiving data in RX_STD_MAILBOX. */
    status |= FLEXCAN_DRV_Receive(CAN1_INST, RX_STD_MAILBOX, &rxStdMsg1);
    status |= FLEXCAN_DRV_Receive(CAN1_INST, RX_EXT_MAILBOX, &rxExtMsg1);

    /* Start receiving data in RX_STD_MAILBOX. */
    status |= FLEXCAN_DRV_Receive(CAN2_INST, RX_STD_MAILBOX, &rxStdMsg2);
    status |= FLEXCAN_DRV_Receive(CAN2_INST, RX_EXT_MAILBOX, &rxExtMsg2);
    ...
    while (1)
    {
        ...
        if (FLEXCAN_DRV_GetTransferStatus(CAN1_INST, TX_MAILBOX) != STATUS_BUSY)
        {
            if (txMsg1.data[0] & 1)
            {
                status |= FLEXCAN_DRV_Send(CAN1_INST, TX_MAILBOX, &txMbStdInfo1, TX_MSG_ID, txMsg1.data);
            }
            else
            {
                status |= FLEXCAN_DRV_Send(CAN1_INST, TX_EXT_MAILBOX, &txMbExtInfo1, TX_EXTMSG_ID, txMsg1.data);
            }
        }
        if (FLEXCAN_DRV_GetTransferStatus(CAN1_INST, RX_STD_MAILBOX) != STATUS_BUSY)
        {
            ...
            PRINTF("CAN1 Rx length= %d, id = %x\n", rxStdMsg1.dataLen, rxStdMsg1.msgId);
            for (int i = 0; i < rxStdMsg1.dataLen; i++)
            {
                PRINTF("%x ", rxStdMsg1.data[i]);
                if (rxStdMsg1.data[i] != txMsg2.data[i])
                {
                    canError = true;
                    PRINTF("\nCAN1 receive mismatch. Expected %x, actual %x.\n", txMsg2.data[i], rxStdMsg1.data[i]);
                }
            }
            PRINTF("\n");
            /* Start receiving data in RX_STD_MAILBOX. */
        ...
        if (FLEXCAN_DRV_GetTransferStatus(CAN1_INST, RX_EXT_MAILBOX) != STATUS_BUSY)
        {
            can1RxFrameCnt++;
            LED_B_TOGGLE();
            /* Print the received data information */
            PRINTF("CAN1 Rx length= %d, id = %x\n", rxExtMsg1.dataLen, rxExtMsg1.msgId);
            for (int i = 0; i < rxExtMsg1.dataLen; i++)
            {
                PRINTF("%x ", rxExtMsg1.data[i]);
                if (rxExtMsg1.data[i] != txMsg2.data[i])
                {
                    canError = true;
                    PRINTF("\nCAN1 receive mismatch. Expected %x, actual %x.\n", txMsg2.data[i], rxExtMsg1.data[i]);
                }
            }
            PRINTF("\n");
            /* Start receiving data in RX_STD_MAILBOX. */
        ...
        if (FLEXCAN_DRV_GetTransferStatus(CAN2_INST, TX_MAILBOX) != STATUS_BUSY)
        {
            if (txMsg2.data[0] & 1)
            {
                status |= FLEXCAN_DRV_Send(CAN2_INST, TX_MAILBOX, &txMbStdInfo2, TX_MSG_ID, txMsg2.data);
            }
            else
            {
                status |= FLEXCAN_DRV_Send(CAN2_INST, TX_EXT_MAILBOX, &txMbExtInfo2, TX_EXTMSG_ID, txMsg2.data);
            }
        }
        if (FLEXCAN_DRV_GetTransferStatus(CAN2_INST, RX_STD_MAILBOX) != STATUS_BUSY)
        {
            can2RxFrameCnt++;
            LED_R_TOGGLE();
            /* Print the received data information */
            PRINTF("CAN2 Rx length= %d, id = %x\n", rxStdMsg2.dataLen, rxStdMsg2.msgId);
            for (int i = 0; i < rxStdMsg2.dataLen; i++)
            {
                PRINTF("%x ", rxStdMsg2.data[i]);
                if (rxStdMsg2.data[i] != txMsg1.data[i])
                {
                    canError = true;
                    PRINTF("\nCAN2 receive mismatch. Expected %x, actual %x.\n", txMsg1.data[i], rxStdMsg2.data[i]);
                }
            }
            PRINTF("\n");
            /* Start receiving data in RX_STD_MAILBOX. */
        ...
        if (FLEXCAN_DRV_GetTransferStatus(CAN2_INST, RX_EXT_MAILBOX) != STATUS_BUSY)
        {
            can2RxFrameCnt++;
            LED_R_TOGGLE();
            /* Print the received data information */
            PRINTF("CAN2 Rx length= %d, id = %x\n", rxExtMsg2.dataLen, rxExtMsg2.msgId);
            for (int i = 0; i < rxExtMsg2.dataLen; i++)
            {
                PRINTF("%x ", rxExtMsg2.data[i]);
                if (rxExtMsg2.data[i] != txMsg1.data[i])
                {
                    canError = true;
                    PRINTF("\nCAN2 receive mismatch. Expected %x, actual %x.\n", txMsg1.data[i], rxExtMsg2.data[i]);
                }
            }
            PRINTF("\n");
            /* Start receiving data in RX_STD_MAILBOX. */
        ...
        {
            canError = true;
            PRINTF("FlexCAN Demo Timeout! Please check connection\n");
        }
    }
    ...
    if (!canError)
    {
        PRINTF("FlexCAN Demo Pass!\n");
    }
    else
    {
        PRINTF("FlexCAN Demo Fail!\n");
    }
    FLEXCAN_DRV_Deinit(CAN1_INST);
    FLEXCAN_DRV_Deinit(CAN2_INST);
    PRINTF("FlexCAN Demo End.\n");
    /* USER CODE END 2 */
