    ...
    switch (eventType)
    {
        case FLEXCAN_EVENT_RX_COMPLETE:
        {
            if (buffIdx == RX_STD_MAILBOX)
            {
                ...
                else if (rxStdMsg.data[0] == 0x02U)
                {
                    sys_mode_sele = POWERDOWN_MODE;
                }
                else
                {}
                LED6_TOGGLE();
...
int main(void)
{
     ...
     * User can choose the standby or powerdown mode by standard can message received. 
    ...
    PRINTF(" Flexcan wake up demo restart\r\n");
...
static void APP_Set_Mode_Fun(void)
{
    switch (sys_mode_sele)
    {
        case STANDBY_MODE:
        {
            ...
            /* Clear pin */
            PINS_DRV_SetMuxModeSel(PCTRLB, 4, PCTRL_PIN_DISABLED);
            PINS_DRV_SetMuxModeSel(PCTRLB, 5, PCTRL_PIN_DISABLED);
            PINS_DRV_SetMuxModeSel(PCTRLE, 8, PCTRL_PIN_DISABLED);
            PINS_DRV_SetMuxModeSel(PCTRLE, 12, PCTRL_PIN_DISABLED);
            PINS_DRV_SetMuxModeSel(PCTRLD, 17, PCTRL_PIN_DISABLED);

            /* Enter into Standby mode */
            POWER_SYS_SetMode(sys_mode_sele, POWER_MANAGER_POLICY_AGREEMENT);

            /* Set pin */
            PINS_DRV_SetMuxModeSel(PCTRLB, 4, PCTRL_MUX_AS_GPIO);
            PINS_DRV_SetMuxModeSel(PCTRLB, 5, PCTRL_MUX_AS_GPIO);
            PINS_DRV_SetMuxModeSel(PCTRLE, 8, PCTRL_MUX_AS_GPIO);
            PINS_DRV_SetMuxModeSel(PCTRLE, 12, PCTRL_MUX_ALT2);
            PINS_DRV_SetMuxModeSel(PCTRLD, 17, PCTRL_MUX_ALT2);

            /* clear CAN0_RX(PTE4) pin wakeup flag */
            WKU_ClearPinWakeupFlag(WKU, wkup_pinsConfig0->hwChannel);
        }
        break;

        case POWERDOWN_MODE:
        {
            PRINTF("<----goto powerdown mode ---->\n\r");
