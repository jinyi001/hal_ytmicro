...
The application is a demonstration of the use of the Flexcan on the YTM32B1MC0 FlexCAN1 channel.
- **Flexcan**: 
  - Use **TX_MAILBOX** defined mailbox number(mailbox-1) to send a standard frame message with id = **TX_MSG_ID**(0x103) every 100ms.
  - Use **RX_STD_MAILBOX** defined mailbox number(mailbox-0) to receive a message id = **RX_STD_MSG_ID**(0x102).
  - Use **RX_EXT_MAILBOX** defined mailbox number(mailbox-2) to receive a message id = **RX_EXT_MSG_ID**(0x70012).
