              ...
              >
                ...
                >
                  ...
                  <el-form-item
                    ...
                    :prop="`timerChannelConfig[${i}].hwChannelId`"
                    :rules="hwChannelIdRule"
                  >
                    <el-input v-model.number="item.timerChannelConfig[i].hwChannelId" />
                  </el-form-item>

                  <!-- Channel counter final value -->
                  <el-form-item
                    label="Final value of channel counter"
                    :prop="`timerChannelConfig[${i}].finalValue`"
                  >
                    <el-input v-model.number="item.timerChannelConfig[i].finalValue" />
                  </el-form-item>

                  <!-- clock source prescaler -->
                  <el-form-item
                    label="Clock source prescaler"
                    :prop="`timerChannelConfig[${i}].clkPrs`"
                  >
                    <el-select
                      v-model.number="item.timerChannelConfig[i].clkPrs"
                      style="width:100%"
                    >
                      <el-option
                        label="1"
                  ...
                  <el-form-item
                    label="Reload point"
                    :prop="`timerChannelConfig[${i}].reloadSrc`"
                  >
                    <el-select
                      v-model.number="item.timerChannelConfig[i].reloadSrc"
                      style="width:100%"
                    >
                      <el-option
                  ...
                  <!-- hardware trigger enable to start counter -->
                  <el-form-item
                    label="Hardware trigger enable"
                    :prop="`timerChannelConfig[${i}].hwTrigEn`"
                  >
                    <el-checkbox v-model.number="item.timerChannelConfig[i].hwTrigEn" />
                  </el-form-item>

                  <!-- debug mode -->
                  <el-form-item
                    label="Debug mode"
                    :prop="`timerChannelConfig[${i}].debugMode`"
                  >
                    <el-checkbox v-model.number="item.timerChannelConfig[i].debugMode" />
                  </el-form-item>

                  <!-- Count mode -->
                  <el-form-item
                    label="Count mode"
                    :prop="`timerChannelConfig[${i}].countMode`"
                  >
                    <el-select
                      v-model.number="item.timerChannelConfig[i].countMode"
                      style="width:100%"
                    >
                      <el-option
                  ...
                  <el-form-item
                    ...
                    :prop="`timerChannelConfig[${i}].timerOverflowIntEn`"
                  >
                    <el-checkbox v-model.number="item.timerChannelConfig[i].timerOverflowIntEn" />
                  </el-form-item>
                </el-collapse-item>
              </el-collapse>
            </el-form>
...
const defMasterParam = {
  ...
  readonly: false,
  channelNum: 1,
  timerChannelConfig: [chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef, chDef],
}
...
function hwChannelId0RuleCheck(rule: any, value: any, callback: any, source: any) {
  ...
  for (let i = 0; i < val.channelNum; i++) {
    if (cnt[val.timerChannelConfig[i].hwChannelId] == undefined) {
      cnt[val.timerChannelConfig[i].hwChannelId] = 1
    } else {
      cnt[val.timerChannelConfig[i].hwChannelId]++
    }
  }
